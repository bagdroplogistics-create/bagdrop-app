...
SMTP_FROM = os.environ.get("SMTP_FROM", SMTP_USER or "noreply@bagdrop.co")
...
def send_booking_email(b: dict) -> None:
    """Best-effort send. Silently no-ops if SMTP not configured."""
    if not (SMTP_HOST and SMTP_USER and SMTP_PASS):
        logger.info("SMTP not configured - skipping email for booking %s", b.get("code"))
        return
    try:
        msg = EmailMessage()
        msg["Subject"] = f"[Bagdrop] New Booking {b.get('code')} - {b.get('service_title')}"
        msg["From"] = SMTP_FROM
        msg["To"] = ADMIN_EMAIL
        if b.get("email"):
            msg["Cc"] = b["email"]
        ...
        if SMTP_PORT == 465:
            ...
            with smtplib.SMTP(SMTP_HOST, SMTP_PORT, timeout=15) as s:
                ...
                s.login(SMTP_USER, SMTP_PASS)
                s.send_message(msg)
        logger.info("Sent booking email for %s to %s", b.get("code"), ADMIN_EMAIL)
    except Exception as e:
        logger.error("Failed to send booking email for %s: %s", b.get("code"), e)
...
async def find_booking_by_any_id(identifier: str) -> Optional[dict]:
    """Find by short code (BDxxxxx) or uuid id."""
    doc = await db.bookings.find_one({"code": identifier.upper()})
    if doc:
        return doc
    return await db.bookings.find_one({"id": identifier})
...
async def create_booking(payload: BookingCreate, background: BackgroundTasks):
    ...
    for _ in range(5):
        exists = await db.bookings.find_one({"code": code})
        if not exists:
            break
        code = generate_booking_code()

    booking = Booking(**payload.dict(), code=code)
    await db.bookings.insert_one(booking.dict())
    # Fire-and-forget email to admin (and customer if email provided)
    background.add_task(send_booking_email, booking.dict())
    return booking
...
@api_router.patch("/bookings/{identifier}/status", response_model=Booking)
async def update_status(identifier: str, body: StatusUpdate):
    doc = await find_booking_by_any_id(identifier)
    if not doc:
        raise HTTPException(status_code=404, detail="Booking not found")
    ...
    if body.stage_index is not None:
        idx = max(0, min(body.stage_index, len(STATUS_FLOW) - 1))
        update["stage_index"] = idx
        update["status"] = STATUS_FLOW[idx]
    elif body.status is not None:
        update["status"] = body.status
        if body.status in STATUS_FLOW:
            update["stage_index"] = STATUS_FLOW.index(body.status)

    await db.bookings.update_one({"id": doc["id"]}, {"$set": update})
